...
void main() {
  ...
  group('of', () {
    test('names the near days rather than dating them', () {
      expect(DayLabel.of(DateTime.utc(2026, 8, 18), today: today), 'Today');
      expect(DayLabel.of(DateTime.utc(2026, 8, 17), today: today), 'Yesterday');
      expect(DayLabel.of(DateTime.utc(2026, 8, 19), today: today), 'Tomorrow');
    });

    test('drops the year while the day is in it', () {
      expect(
        DayLabel.of(DateTime.utc(2026, 8, 11), today: today),
        'Tue 11 Aug',
      );
    });
    ...
    test('shows the year once the day is outside it', () {
      expect(
        DayLabel.of(DateTime.utc(2025, 12, 25), today: today),
        'Thu 25 Dec 2025',
      );
    });
  ...
  group('full', () {
    test('writes the weekday, day, month and year', () {
      expect(DayLabel.full(DateTime.utc(2026, 1, 5)), 'Mon 5 Jan 2026');
    });

    test('leaves the year off when asked', () {
      expect(
        DayLabel.full(DateTime.utc(2026, 1, 5), withYear: false),
        'Mon 5 Jan',
      );
    });
  });
  ...
  group('names', () {
    test('numbers the weekdays from Monday', () {
      expect(
        [for (var i = 1; i <= 7; i++) DayLabel.weekday(i)],
    ...
    test('numbers the months from January', () {
      expect(
        [for (var i = 1; i <= 12; i++) DayLabel.month(i)],
        [
          'Jan',
          'Feb',
          'Mar',
    ...
    test('matches what DateTime reports for a known date', () {
      ...
      expect(DayLabel.weekday(date.weekday), 'Tue');
      expect(DayLabel.month(date.month), 'Aug');
    });
  });
}

...
- Weekly digest: a one-shot local notification rescheduled on every app open, because its text is a snapshot of who is overdue now. `digestDay` (null = off) and `digestHour` are per-member fields on `households/{hid}/members/{uid}`. A digest with nothing to say is cancelled, not posted. Scheduling is inexact on Android by choice.
